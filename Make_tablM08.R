...
Make_tablM08 <- function(Est1,Typ1,type,month1,month2) {
  ...
  if (Est1=="Employment") {
    title <- "Employment"
    subtitle0 <- paste0(Typ1,"<br>Persons<br>")
  }
  else if (Est1=="Average weekly earnings including overtime") {
    title <- "Average weekly earnings including overtime"
    subtitle0 <- paste0(Typ1,"<br>Dollars<br>")
  }
  else if (Est1=="Average hourly earnings including overtime") {
    title <- "Average hourly earnings including overtime"
    subtitle0 <- paste0(Typ1,"<br>Dollars<br>")
  }
  else if (Est1=="Average weekly hours") {
    title <- "Average weekly hours"
    subtitle0 <- paste0(Typ1,"<br>Hours<br>")
  }
  subtitle1 <- paste0(Typ1)
  subtitle <- case_when(
        type==1 ~ paste0(subtitle0,
          TS[[8]]$Seas,"<br><br>"),
  ...
  gt_tbl <- tab_header(gt_tbl,
      title=md(paste0("**","Table ",TS[[8]]$Bnum,"<br>",title,"**")),
      subtitle=md(paste0(subtitle,"<br><br>")))
  gt_tbl <- tab_source_note(gt_tbl,
      source_note=md(TS[[8]]$Ftnt)) 
