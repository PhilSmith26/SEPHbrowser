# Chart module #1 for SEPH employment by ind and prov, NSA
...
mc01UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    tags$b(tags$span(style="color:blue",HTML(paste0("<h2>Employment ",
      "by industry and province or territory, not seasonally ",
      "adjusted</h2><br>")))),
    ...
    prettyRadioButtons(NS(id,"Geo"),
      ...
      outline=TRUE,inline=TRUE,shape="round",animation="pulse"),
    tags$style(HTML(".selectize-input, .option {color:blue;font-size:26px}")),
    selectInput(NS(id,"naics"),
      label=tags$b(tags$span(style="color:blue; font-size:20px",
      "Choose an industry:")),choices=naics02,width = "100%"),
    fluidRow(column(5,
    ...
    sliderTextInput(NS(id,"Dates"),label= 
      ...
      choices=getChoices(1)[[1]],
      selected=getChoices(1)[[3]],
      dragRange = TRUE,
      width="100%"),
    tags$script(HTML(
      "$('.shiny-input-container:has(input[id=\"idmc01-Dates\"])>label')
      .css({ffontWeight:900,fontSize:'20px',color:'blue'})")),
    plotOutput(NS(id,"chart")) 
  )
}
mc01Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    chartP <- reactive({
      if (Seas2()) {Make_chrtM01(naics2(),Geo2(),Kin2(),"TRUE",type2(),
        month1(),month2(),altTitle(),"")}
      else {(Make_chrtM01(naics2(),Geo2(),Kin2(),"FALSE",type2(),
        month1(),month2(),altTitle(),""))}
    })  
