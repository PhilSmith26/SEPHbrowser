...
Make_tablM07 <- function(Est1,type,month1,month2) {
  ...
  if (Est1=="Employment for all employees") {
    title <- "Employment for all employees"
    subtitle0 <- "Persons<br>"
  }
  else {
    title <- "Average weekly earnings including overtime"
    subtitle0 <- "Dollars<br>"
  }
  subtitle1 <- paste0(Est1)
  subtitle <- case_when(
  ...
  if(type==1) { # Display original data
    ...
    if (Est1=="Employment for all employees") {dec1 <- 0}
    else {dec1 <- 2}    
  ...
  gt_tbl <- tab_header(gt_tbl,
      title=md(paste0("**","Table ",TS[[7]]$Bnum,"<br>",title,"**")),
      subtitle=md(paste0(subtitle,"<br><br>")))
  gt_tbl <- tab_source_note(gt_tbl,
      source_note=md(TS[[7]]$Ftnt)) 
