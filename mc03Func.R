...
e <- seq.Date(TS[[3]]$Strt,TS[[3]]$Endt,by="month")
s <- character()
for (i in 1:length(e)) {
  s[i] <- format(e[i],"%b %Y")
}
...
mc03UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    prettyRadioButtons(NS(id,"Ovr"),
      ...
      "Overtime:")),choices=Ovr1001,bigger=TRUE,
...
mc03Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    output$downloadData1 <- downloadHandler(
      filename=function() {
        paste0("Average_hourly_earnings.png")
      },
    ...
    observeEvent(input$Geo,
      updateSelectInput(session,"naics",label="Choose an industry:",
        choices=unique(filter(q0,GEO==input$Geo,OVR==input$Ovr)$NAICS))
    )
