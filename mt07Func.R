# Table module #7 for SEPH empl and AHE by ind, SA
# August 2, 2021; improved August 9, 2021

mt07UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Tables"))),
    tags$b(tags$span(style="color:blue",HTML(paste0("<h2>Employment and ",
      "average weekly earnings (including overtime) by industry, ",
      "seasonally adjusted</h2><br>")))),
    prettyRadioButtons(NS(id,"Est"),
      tags$b(tags$span(style="color:blue;font-size:20px", 
        "Employment or average weekly earnings:")),choices=est03,bigger=TRUE,
    ...
    sliderTextInput(NS(id,"Dates"),label="Choose starting and ending dates:",
      choices=getChoices(7)[[1]],
      selected=getChoices(7)[[2]],
      dragRange = TRUE,
      width="100%"),
    tags$script(HTML(
      "$('.shiny-input-container:has(input[id=\"idmt07-Dates\"])>label')
      .css({fontFamily:'helvetica',fontWeight:900,fontSize:'20px',color:'blue'})")),    
    htmlOutput(NS(id,"notabl")),
    gt_output(NS(id,"tabl")) 
  )
}
mt07Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    expr <- reactive({
      Make_tablM07(Est2(),type1(),month1(),month2())
    })
