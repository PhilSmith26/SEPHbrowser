...
e <- seq.Date(TS[[11]]$Strt,TS[[11]]$Endt,by="month")
s <- character()
for (i in 1:length(e)) {
  s[i] <- format(e[i],"%b %Y")
}
...
mc11UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    ...
    prettyRadioButtons(NS(id,"Ovt"),
      tags$b(tags$span(style="color:blue;font-size:20px", 
      "Overtime:")),choices=Ovt1001,bigger=TRUE,
      outline=TRUE,inline=TRUE,shape="round",animation="pulse"),
...
mc11Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    output$downloadData1 <- downloadHandler(
      filename=function() {
        paste0("Average_weekly_hours.png")
      },
    ...
    observeEvent(input$Geo,
      updateSelectInput(session,"naics",label="Choose an industry:",
        choices=unique(filter(q0,GEO==input$Geo,OVT==input$Ovt)$NAICS))
    )
