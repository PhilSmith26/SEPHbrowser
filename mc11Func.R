# Chart module #11 for SEPH AWH by ind and prov, NSA
...
mc11UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    ...
    sliderTextInput(NS(id,"Dates"),label= 
      ...
      selected=getChoices(11)[[3]],
      dragRange = TRUE,
      width="100%"),
    tags$script(HTML(
      "$('.shiny-input-container:has(input[id=\"idmc11-Dates\"])>label')
...
mc11Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    chartP <- reactive({
      if (Seas2()) {Make_chrtM11(naics2(),Geo2(),Ovt2(),"TRUE",type2(),
        month1(),month2(),altTitle(),"")}
      else {(Make_chrtM11(naics2(),Geo2(),Ovt2(),"FALSE",type2(),
        month1(),month2(),altTitle(),""))}
    })  
