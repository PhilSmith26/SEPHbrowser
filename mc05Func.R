# Chart module #5 for SEPH standard work week for salaried empl by ind and prov, NSA
# August 2, 2021; improved August 10, 2021

mc05UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    tags$b(tags$span(style="color:blue",HTML(paste0("<h2>Standard ",
      "work week for salaried employees by industry and province or ",
      "territory, not seasonally adjusted</h2><br>")))),
    ...
    selectInput(NS(id,"naics"),
      ...
      "Choose an industry:")),choices=naics05,width = "100%"),
    fluidRow(column(5,
    ...
    sliderTextInput(NS(id,"Dates"),label= 
      ...
      choices=getChoices(5)[[1]],
      selected=getChoices(5)[[3]],
      dragRange = TRUE,
      width="100%"),
    tags$script(HTML(
      "$('.shiny-input-container:has(input[id=\"idmc05-Dates\"])>label')
      .css({ffontWeight:900,fontSize:'20px',color:'blue'})")),
    plotOutput(NS(id,"chart")) 
  )
}
mc05Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    chartP <- reactive({
      if (Seas2()) {Make_chrtM05(naics2(),Geo2(),"TRUE",type2(),
        month1(),month2(),altTitle(),"")}
      else {(Make_chrtM05(naics2(),Geo2(),"FALSE",type2(),
        month1(),month2(),altTitle(),""))}
    })  
