...
mc06UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    tags$b(tags$span(style="color:blue",HTML(paste0("<h2>Fixed weighted ",
      "index of average hourly earnings, by industry and province or ",
    ...
    selectInput(NS(id,"naics"),
      ...
      "Choose an industry:")),choices=naics04,width = "100%"),
    fluidRow(column(5,
    ...
    sliderTextInput(NS(id,"Dates"),label= 
      ...
      choices=getChoices(6)[[1]],
      selected=getChoices(6)[[3]],
      dragRange = TRUE,
      width="100%"),
    tags$script(HTML(
