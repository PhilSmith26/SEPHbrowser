...
Make_tablM09 <- function(Geo1,type,month1,month2) {
  ...
  q0 <- select(q0,Date,EST,VALUE)
  q0 <- pivot_wider(q0,names_from=EST,values_from=VALUE)
  subtitle1 <- paste0(Geo1,"<br>Persons (lines 1-4), dollars (lines 5-9) or hours (lines 10-11)<br>")
  subtitle <- case_when(
  ...
  gt_tbl <- tab_header(gt_tbl,
      title=md(paste0("**","Table ",TS[[9]]$Bnum,"<br>",TS[[9]]$Titl,"**")),
      subtitle=md(paste0(subtitle,"<br><br>")))
  gt_tbl <- tab_source_note(gt_tbl,
      source_note=md(TS[[9]]$Ftnt)) 
