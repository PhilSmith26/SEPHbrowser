...
naics1001 <- c(
  ...
  "Internet service providers, web search portals [5181]",                                                             
...
e <- seq.Date(TS[[2]]$Strt,TS[[2]]$Endt,by="month")
s <- character()
for (i in 1:length(e)) {
  s[i] <- format(e[i],"%b %Y")
}
...
mc02UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    prettyRadioButtons(NS(id,"Kin"),
      ...
      "Choose a type of employee:")),choices=Kin1000,bigger=TRUE,
      outline=TRUE,inline=TRUE,shape="round",animation="pulse"),
    prettyRadioButtons(NS(id,"Ovr"),
      tags$b(tags$span(style="color:blue;font-size:20px", 
      "Overtime:")),choices=Ovr1000,bigger=TRUE,
      outline=TRUE,inline=TRUE,shape="round",animation="pulse"),
...
mc02Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    output$downloadData1 <- downloadHandler(
      filename=function() {
        paste0("Average_weekly_earnings.png")
      },
    ...
    observeEvent(input$Geo,
      updateSelectInput(session,"naics",label="Choose an industry:",
        choices=unique(filter(q0,GEO==input$Geo,KIN==input$Kin,OVR==input$Ovr)$NAICS))
    )
    observeEvent(input$Kin,
      updateSelectInput(session,"naics",label="Choose an industry:",
        choices=unique(filter(q0,GEO==input$Geo,KIN==input$Kin,OVR==input$Ovr)$NAICS))
    )
    observeEvent(input$Ovt,
      updateSelectInput(session,"naics",label="Choose an industry:",
        choices=unique(filter(q0,GEO==input$Geo,KIN==input$Kin,OVR==input$Ovr)$NAICS))
    )
  })
