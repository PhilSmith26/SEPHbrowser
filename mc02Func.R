# Chart module #2 for SEPH AWE by ind and prov, NSA
...
mc02UI <- function(id) {
  tabPanel(tags$b(tags$span(style="color:blue", HTML("Charts"))),
    ...
    sliderTextInput(NS(id,"Dates"),label= 
      ...
      selected=getChoices(2)[[3]],
      dragRange = TRUE,
      width="100%"),
    tags$script(HTML(
      "$('.shiny-input-container:has(input[id=\"idmc02-Dates\"])>label')
      .css({ffontWeight:900,fontSize:'20px',color:'blue'})")),
    plotOutput(NS(id,"chart")) 
  )
}
mc02Server <- function(id) {
  moduleServer(id,function(input,output,session) {
    ...
    chartP <- reactive({
      if (Seas2()) {Make_chrtM02(naics2(),Geo2(),Kin2(),Ovr2(),"TRUE",type2(),
        month1(),month2(),altTitle(),"")}
      else {(Make_chrtM02(naics2(),Geo2(),Kin2(),Ovr2(),"FALSE",type2(),
        month1(),month2(),altTitle(),""))}
    })  
